...
userSchema.methods.isOnTrial = function () {
  return (
    this.isTrialActive && this.trialEndsAt && this.trialEndsAt > new Date()
  );
};
...
userSchema.methods.updateGoogleTokens = function (tokens) {
  if (!tokens) return this.save();
  if (tokens.access_token) this.googleAccessToken = tokens.access_token;
  if (tokens.refresh_token) this.googleRefreshToken = tokens.refresh_token;
  if (tokens.expiry_date) this.googleTokenExpiry = new Date(tokens.expiry_date);
  return this.save({ validateBeforeSave: false });
};

// Instance method to upgrade subscription
userSchema.methods.upgradeSubscription = function (
  planId,
...
// Instance method to update X (Twitter) tokens
userSchema.methods.updateXTokens = function (tokens) {
  if (!tokens) return this.save({ validateBeforeSave: false });
  if (tokens.accessToken) this.xAccessToken = tokens.accessToken;
  if (tokens.refreshToken) this.xRefreshToken = tokens.refreshToken;
  if (tokens.expiresIn)
    this.xTokenExpiry = new Date(Date.now() + tokens.expiresIn * 1000);
  if (Array.isArray(tokens.scopes))
    this.xScopes = Array.from(
      new Set([...(this.xScopes || []), ...tokens.scopes]),
    );
  return this.save({ validateBeforeSave: false });
};
