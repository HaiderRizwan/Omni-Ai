...
const Sidebar = ({
  ...
  const loadPhylloScript = () =>
    new Promise((resolve, reject) => {
      ...
      script.src =
        process.env.REACT_APP_PHYLLO_CONNECT_URL ||
  ...
  const startPhylloConnect = async () => {
    try {
      ...
      console.log(
        ...
        apiBase,
        "env ->",
        process.env.REACT_APP_PHYLLO_ENV,
      );
      ...
      if (!window.PhylloConnect)
        throw new Error("Phyllo Connect SDK not available");
  ...
  const handleSubscribe = async () => {
    try {
      ...
      const planId =
        Array.isArray(plansJson.data) && plansJson.data[0]
          ? plansJson.data[0]._id
          : undefined;
      ...
      if (btn) {
        ...
        btn.className =
          "w-full mb-4 flex items-center justify-center gap-2 rounded-xl bg-green-600 text-white font-medium py-3";
      }
  ...
  // Get chats strictly from server, ignoring any local storage chats
  const getChatsInBackendOrder = (toolId) => {
    ...
    const serverChatType = Object.keys(chatTypeMap).find(
      (type) => chatTypeMap[type] === toolId,
    );
    if (!serverChatType) {
      return [];
    }
    ...
    // Map server chats to sidebar items
    const mapped = serverChatsForTool.map((serverChat) => ({
      id: `server-${serverChat._id}`,
      serverId: serverChat._id,
      title: serverChat.title,
      timestamp: new Date(
        serverChat.createdAt || serverChat.updatedAt,
      ).getTime(),
  ...
  const allServerChatsForDisplay = tools
    .filter((t) =>
      ["chat", "image", "video", "avatar", "avatarVideo"].includes(t.id),
    )
