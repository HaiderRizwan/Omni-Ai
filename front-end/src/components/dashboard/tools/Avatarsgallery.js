    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">Error Loading Avatars</h3>
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        loading="lazy"
                        onError={(e) => {
                          console.error('Image failed to load:', getImageUrl(avatar._id), avatar);
                          
                          // Try to fetch the image directly to debug the issue
                          fetch(getImageUrl(avatar._id))
                            .then(response => {
                        ...
                        onLoad={() => {
                          console.log('Image loaded successfully:', getImageUrl(avatar._id));
                        }}
                      />
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 border-b border-white/10">
                <h3 className="text-white font-medium truncate flex-1 mr-4" title={selectedAvatar.prompt}>
              ...
              <div className="p-4 border-t border-white/10">
                <div className="grid grid-cols-2 gap-4 text-sm text-gray-400">
                  <div>
                    ...
                    <span className="ml-2">{selectedAvatar.width}×{selectedAvatar.height}</span>
                  </div>
                  <div>
                    <span className="text-gray-300">File Size:</span>
                    <span className="ml-2">{formatFileSize(selectedAvatar.size)}</span>
                  </div>
                  <div>
                    <span className="text-gray-300">Format:</span>
                    <span className="ml-2">
                      {selectedAvatar.contentType?.split('/')[1]?.toUpperCase() || 'Unknown'}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-300">Created:</span>
                    <span className="ml-2">{formatDate(selectedAvatar.createdAt)}</span>
                  </div>
                  {selectedAvatar.characterDescription && (
                    <div className="col-span-2">
                      <span className="text-gray-300">Character:</span>
                      <span className="ml-2">{selectedAvatar.characterDescription}</span>
                    </div>
                  )}
                </div>
              </div>
