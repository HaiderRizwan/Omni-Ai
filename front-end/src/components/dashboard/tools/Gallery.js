...
const Gallery = () => {
  ...
  const handleDownload = async (image) => {
    try {
      ...
      push({ message: 'Image download started', type: 'success' });
    } catch (error) {
            ...
            <div className="flex items-center gap-3">
              <div>
                ...
                <p className="text-gray-400 text-sm font-medium mt-1">
                  {sortedImages.length} images generated
                </p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 border-b border-white/10 bg-white/[0.02]">
                <h3 className="text-white font-medium truncate flex-1 mr-4 text-lg" title={selectedImage.prompt}>
              ...
              <div className="p-5 border-t border-white/10 bg-white/[0.02]">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm">
                  <div>
                    ...
                    <span className="text-gray-200 font-medium">{selectedImage.width} × {selectedImage.height}</span>
                  </div>
                  <div>
                    <span className="block text-gray-500 text-xs uppercase tracking-wider mb-1">File Size</span>
                    <span className="text-gray-200 font-medium">{formatFileSize(selectedImage.size)}</span>
                  </div>
                  <div>
                    ...
                    <span className="text-gray-200 font-medium tracking-wide">
                      {selectedImage.contentType?.split('/')[1]?.toUpperCase() || 'PNG'}
                    </span>
                  </div>
                  <div>
                    <span className="block text-gray-500 text-xs uppercase tracking-wider mb-1">Created</span>
                    <span className="text-gray-200 font-medium">{formatDate(selectedImage.createdAt)}</span>
                  </div>
                </div>
              </div>
            </motion.div>
